...
const logger = readonly(createLogger())
...
export function createTranslateReferWords(entrys: Entry[], dictionary: Record<string, WordRo>) {
	const entryNameSet = new Set<string>(entrys.map(e => e.wordName))
 ...
 */
export function createCtx(log: Logger): HookCtx {
	return {
		log: log.info
	}
}
 ...
 * 单对象翻译逻辑
 */
export function doTranslateObject(object: IniObjectRo, dictionary: Record<string, WordRo>) {
	if (!object) throw Error('对象不合法：' + object)
	const words = createTranslateReferWords(object.entry, dictionary)
	for (const name in words) { checkWordHook(words[name]) }
	const ret = objectTranslator(object, words, createCtx(logger))
	return ret
}
 ...
 */
export function findWordByName(wordName: string, dictionary: Record<string, WordRo>) {
	forIn(dictionary, (key, val) => {
		// if ()
	})
}
...
export function checkWordHook(word: WordRo) {
	if (!word.hooks) {
		...
		catch (e: any) { logger.error(`加载${word.fullname}的hooks时出错`, e?.stack || e) }
	}
}
...
export function checkMapperHandlers(mapper: MapperRo) {
	...
	catch (e) { logger.error(`加载${mapper.fullname}的handlers时出错:${e}`) }
}
...
export function doBuild(
	objectsList: IniObjectRo[],
	mappers: Record<string, MapperRo>,
	dictionary: Record<string, WordRo>
) {

	// 1.执行对象翻译步骤
	const translationResults: Record<string, string[][]> = {}
	for (const object of objectsList) {
		const translation = doTranslateObject(object, dictionary)
...
function doMapperHandle(mapperToOutput: Set<MapperRo>) {
	...
	for (const mapper of mapperToOutput) {
		...
		if (!final) {
			logger.warn('输出器缺失final output handler', mapper.name)
			continue
		}
